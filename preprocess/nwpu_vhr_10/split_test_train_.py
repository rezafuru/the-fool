...
mkdir(root_split.__str__())
dir_positives = root_orig / "positives"
dir_negatives = root_orig / "negatives"

paths_positives = list(dir_positives.iterdir())
random.shuffle(paths_positives)
paths_negatives = list(dir_negatives.iterdir())
random.shuffle(paths_negatives)

train_positives, test_positives = split_array(paths_positives, 90)
train_negatives, test_negatives = split_array(paths_negatives, 90)

for sample_path in train_positives:
    ...
    split_label_path_ultralytics = labels_split_ultralytics_train / sample_label_name

    shutil.copy(src=orig_label_path.__str__(), dst=split_label_path.__str__())
    shutil.copy(
        src=orig_ultralytics_path.__str__(), dst=split_label_path_ultralytics.__str__()
    )


for sample_path in test_positives:
    sample_base_name = sample_path.stem
    sample_img_name = f"{sample_base_name}.jpg"
    sample_label_name = f"{sample_base_name}.txt"
...
idx = len(test_positives + train_positives) + 1
for sample_path in train_negatives:
    idx += 1
