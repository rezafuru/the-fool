...
from torchdistill.datasets.coco import (
    ImageToTensor,
    Compose,
    CocoRandomHorizontalFlip,
    get_coco,
)
...
from torchdistill.datasets.wrapper import (
    default_idx2subpath,
    BaseDatasetWrapper,
    CacheableDataset,
    get_dataset_wrapper,
...
def split_dataset(org_dataset, random_split_config, dataset_id, dataset_dict):
    ...
    for sub_dataset, sub_split_params in zip(sub_datasets, sub_splits_config):
        ...
        if (
            hasattr(sub_dataset.dataset, "target_transform")
...
def get_dataset_dict(dataset_config):
    ...
    if dataset_type == "cocodetect":
        ...
        for split_name in dataset_splits_config.keys():
            ...
            transforms = build_transform(
                split_config.get("transforms_params", None), compose_cls=compose_cls
            )
            dataset_dict[split_config["dataset_id"]] = load_coco_dataset(
                split_config["images"],
                split_config["annotations"],
                split_config["annotated_only"],
                split_config.get("random_horizontal_flip", None),
                is_segment,
                transforms,
                split_config.get("jpeg_quality", None),
            )
...
def build_data_loader(dataset, data_loader_config, distributed, accelerator=None):
    ...
    if isinstance(dataset_wrapper_config, dict) and len(dataset_wrapper_config) > 0:
        ...
        dataset = CacheableDataset(
            dataset, cache_dir_path, idx2subpath_func=default_idx2subpath
        )
    ...
    sampler = (
        ...
        if data_loader_config.get("random_sample", False)
...
def build_data_loaders(
    dataset_dict, data_loader_configs, distributed, accelerator=None
):
    ...
    for data_loader_config in data_loader_configs:
        ...
        data_loader = (
            ...
            if dataset_id is None or dataset_id not in dataset_dict
                ...
                dataset_dict[dataset_id], data_loader_config, distributed, accelerator
            )
        )
