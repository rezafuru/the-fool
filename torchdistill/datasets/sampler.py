...
def compute_aspect_ratios(dataset, indices=None):
    target_dataset = (
        dataset.org_dataset if isinstance(dataset, BaseDatasetWrapper) else dataset
    )
...
def get_batch_sampler(dataset, class_name, *args, **kwargs):
    ...
    batch_sampler_cls = (
        BatchSampler
        if class_name == "BatchSampler"
        else BATCH_SAMPLER_CLASS_DICT[class_name]
    )
    if batch_sampler_cls == GroupedBatchSampler:
        group_ids = create_aspect_ratio_groups(
            dataset, k=kwargs.pop("aspect_ratio_group_factor")
        )
