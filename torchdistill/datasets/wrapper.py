...
class CacheableDataset(BaseDatasetWrapper):
    ...
    def __getitem__(self, index):
        ...
        cache_file_path = os.path.join(
            self.cache_dir_path, self.idx2subath_func(index) + self.ext
        )
...
class ContrastiveDataset(BaseDatasetWrapper):
    def __init__(self, org_dataset, num_negative_samples, mode, ratio):
        ...
        self.cls_positives = [
            np.asarray(self.cls_positives[i]) for i in range(num_classes)
        ]
        self.cls_negatives = [
            np.asarray(self.cls_negatives[i]) for i in range(num_classes)
        ]
        if 0 < ratio < 1:
            n = int(len(self.cls_negatives[0]) * ratio)
            self.cls_negatives = [
                np.random.permutation(self.cls_negatives[i])[0:n]
                for i in range(num_classes)
            ]
    ...
    def __getitem__(self, index):
        ...
        replace = (
            True
            if self.num_negative_samples > len(self.cls_negatives[target])
            else False
        )
        neg_idx = np.random.choice(
            self.cls_negatives[target], self.num_negative_samples, replace=replace
        )
