...
class TrainingBox(nn.Module):
    ...
    def setup_model(self, model_config):
        unwrapped_org_model = (
            self.org_model.module
            if check_if_wrapped(self.org_model)
            else self.org_model
        )
        ...
        if len(model_config) > 0 or (len(model_config) == 0 and self.model is None):
            model_type = "original"
            special_model = build_special_module(
                model_config,
                student_model=unwrapped_org_model,
                device=self.device,
        ...
        self.model_any_frozen = len(
            model_config.get("frozen_modules", list())
        ) > 0 or not model_config.get("requires_grad", True)
        self.target_model_pairs.extend(
            set_hooks(self.model, ref_model, model_config, self.model_io_dict)
        )
        self.model_forward_proc = get_forward_proc_func(
            model_config.get("forward_proc", None)
        )
    ...
    def setup(self, train_config):
        ...
        model_unused_parameters = model_config.get(
            "find_unused_parameters", self.model_any_frozen
        )
        ...
        if self.accelerator is not None:
            ...
            self.model, self.optimizer = amp.initialize(
                self.model, self.optimizer, opt_level=apex_config["opt_level"]
            )
            self.apex = True
    ...
    def forward(self, sample_batch, targets, supp_dict, return_io_dict=False):
        with torch.autocast(
            ...
            model_outputs = self.model_forward_proc(
                self.model, sample_batch, targets, supp_dict
            )
            extracted_model_io_dict = extract_io_dict(self.model_io_dict, self.device)
            output_dict = {"model_outputs": model_outputs}
            if isinstance(self.model, SpecialModule):
                self.model.post_forward(extracted_model_io_dict)
            if not extracted_model_io_dict:
                total_loss = self.criterion(model_outputs, targets)
            else:
                ...
                org_loss_dict = self.extract_org_loss(
                    ...
                    teacher_outputs,
                    targets,
                    uses_teacher_output=False,
                    supp_dict=supp_dict,
                )
                update_io_dict(
                    extracted_model_io_dict,
                    extract_io_dict(self.model_io_dict, self.device),
                )
