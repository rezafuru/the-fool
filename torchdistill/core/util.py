...
def set_hooks(model, unwrapped_org_model, model_config, io_dict):
    ...
    for target_module_path in input_module_path_set.union(output_module_path_set):
        ...
        handle = register_forward_hook_with_dict(
            target_module, target_module_path, requires_input, requires_output, io_dict
        )
...
def wrap_model(
    ...
    if (
        wrapper is not None
        and device.type.startswith("cuda")
        and not check_if_wrapped(model)
    ):
...
def tensor2numpy2tensor(data, device):
    ...
    if isinstance(data, torch.Tensor):
        ...
        return elem_type(
            *(tensor2numpy2tensor(samples, device) for samples in zip(*data))
        )
    elif isinstance(data, (list, tuple)):
...
def extract_io_dict(model_io_dict, target_device):
    ...
    for module_path, module_io_dict in model_io_dict.items():
        ...
        for io_type in list(module_io_dict.keys()):
            ...
            gathered_obj = (
                gather(values, target_device)
                if uses_cuda and len(values) > 1
                else values[-1]
            )
