...
from torchdistill.datasets.util import build_data_loaders, build_data_loader
...
from train.scaler import GradScaleMockWrapper
...
class DistillationBox(nn.Module):
    def setup_data_loaders(self, train_config):
        ...
        val_data_loaders = train_config.get("val_data_loaders", dict())
        if val_data_loaders:
            train_data_loader = (
                build_data_loader(
                    self.dataset_dict[train_data_loader_config["dataset_id"]],
                    train_data_loader_config,
                    self.distributed,
                    self.accelerator,
                )
                if "dataset_id" in train_data_loader_config
                else None
            )
            val_data_loader = {
                path: build_data_loader(
                    self.dataset_dict[loader_config["dataset_id"]],
                    loader_config,
                    self.distributed,
                    self.accelerator,
                )
                for path, loader_config in val_data_loaders.items()
            }
            # train_data_loader, *val_data_loader = build_data_loaders(
            #     self.dataset_dict,
            #     [train_data_loader_config, *val_data_loader_config],
            #     self.distributed,
            #     self.accelerator,
            # )
        else:
    ...
    def setup_teacher_student_models(self, teacher_config, student_config):
        unwrapped_org_teacher_model = (
            self.org_teacher_model.module
            if check_if_wrapped(self.org_teacher_model)
            else self.org_teacher_model
        )
        unwrapped_org_student_model = (
            self.org_student_model.module
            if check_if_wrapped(self.org_student_model)
        ...
        if len(teacher_config) > 0 or (
            len(teacher_config) == 0 and self.teacher_model is None
        ):
            model_type = "original"
            ...
            self.teacher_model = redesign_model(
                teacher_ref_model, teacher_config, "teacher", model_type
            )

        if len(student_config) > 0 or (
            len(student_config) == 0 and self.student_model is None
        ):
            model_type = "original"
            special_student_model = build_special_module(
                student_config,
            ...
            self.student_model = redesign_model(
                student_ref_model, student_config, "student", model_type
            )

        self.teacher_any_frozen = len(
            teacher_config.get("frozen_modules", list())
        ) > 0 or not teacher_config.get("requires_grad", True)
        self.student_any_frozen = len(
            student_config.get("frozen_modules", list())
        ) > 0 or not student_config.get("requires_grad", True)
    ...
    def setup_loss(self, train_config):
        ...
        org_criterion_config = (
            org_term_config.get("criterion", dict())
            if isinstance(org_term_config, dict)
            else None
        )
        self.org_criterion = (
            None
            if org_criterion_config is None or len(org_criterion_config) == 0
            else get_single_loss(org_criterion_config).to(self.device)
        )
        self.criterion = get_custom_loss(criterion_config)
        if hasattr(self.criterion, "term_dict"):
            for name, (criterion, rate) in self.criterion.term_dict.items():
                self.criterion.term_dict[name] = (criterion.to(self.device), rate)
        logger.info(self.criterion)
        self.uses_teacher_output = self.org_criterion is not None and isinstance(
            self.org_criterion, tuple(ORG_LOSS_LIST)
        )
    ...
    def setup(self, train_config):
        ...
        if len(optim_config) > 0:
            ...
            if len(module_wise_params_configs) > 0:
                ...
                for module_wise_params_config in module_wise_params_configs:
                    ...
                    target_model = (
                        self.teacher_model
                        if module_wise_params_config.get("is_teacher", False)
        ...
        if scheduler_config is not None and len(scheduler_config) > 0:
            self.lr_scheduler = get_scheduler(
                self.optimizer, scheduler_config["type"], scheduler_config["params"]
            )
        ...
        if self.accelerator is not None:
            ...
            self.student_model, self.optimizer = amp.initialize(
                self.student_model, self.optimizer, opt_level=apex_config["opt_level"]
            )
            self.apex = True
    ...
    def __init__(
        ...
        (
            ...
            self.extract_org_loss,
        ) = (None, None, None, None)
        self.teacher_updatable, self.teacher_any_frozen, self.student_any_frozen = (
            None,
            None,
            None,
        )
        ...
        self.enable_mp = train_config.get("enable_mp", False)
        logger.info(f"Enabling AMP: {self.enable_mp}")
        self.scaler = GradScaleMockWrapper(GradScaler() if self.enable_mp else None)
    ...
    def get_teacher_output(self, sample_batch, targets, supp_dict):
        ...
        if cached_data is not None and isinstance(cached_data, dict):
            ...
            if device.type != "cpu":
                ...
                cached_extracted_teacher_output_dict = change_device(
                    cached_extracted_teacher_output_dict, device
                )
        ...
        if teacher_outputs is None:
            if self.teacher_updatable:
                teacher_outputs = self.teacher_forward_proc(
                    self.teacher_model, sample_batch, targets, supp_dict
                )
            else:
                with torch.no_grad():
                    teacher_outputs = self.teacher_forward_proc(
        ...
        if cached_extracted_teacher_output_dict is not None:
            if isinstance(self.teacher_model, SpecialModule) or (
                check_if_wrapped(self.teacher_model)
                and isinstance(self.teacher_model.module, SpecialModule)
            ):
            ...
            extracted_teacher_io_dict = extract_io_dict(
                self.teacher_io_dict, self.device
            )
        ...
        teacher_io_dict4cache = (
            copy.deepcopy(self.teacher_io_dict)
            if self.teacher_updatable
            and isinstance(cache_file_paths, (list, tuple)) is not None
            else None
        )
        ...
        update_io_dict(
            ...
            extract_io_dict(self.teacher_io_dict, self.device),
        )
        ...
        if isinstance(cache_file_paths, (list, tuple)):
            ...
            for i, (teacher_output, cache_file_path) in enumerate(
                zip(teacher_outputs.cpu().numpy(), cache_file_paths)
            ):
                sub_dict = extract_sub_model_output_dict(teacher_io_dict4cache, i)
                sub_dict = tensor2numpy2tensor(sub_dict, cpu_device)
                cache_dict = {
                    "teacher_outputs": torch.Tensor(teacher_output),
                    "extracted_outputs": sub_dict,
                }
                make_parent_dirs(cache_file_path)
                torch.save(cache_dict, cache_file_path)
        return teacher_outputs, extracted_teacher_io_dict

    def forward(self, sample_batch, targets, supp_dict, return_io_dict=False):
        with torch.autocast(
            device_type="cuda", dtype=torch.float16, enabled=self.enable_mp
        ):
            teacher_outputs, extracted_teacher_io_dict = self.get_teacher_output(
                sample_batch, targets, supp_dict=supp_dict
            )
            student_outputs = self.student_forward_proc(
                self.student_model, sample_batch, targets, supp_dict
            )
            extracted_student_io_dict = extract_io_dict(
                self.student_io_dict, self.device
            )
            ...
            update_io_dict(
                extracted_student_io_dict,
                extract_io_dict(self.student_io_dict, self.device),
            )
            if teacher_outputs is not None and not extracted_teacher_io_dict:
                extracted_teacher_io_dict["full"] = {"output": teacher_outputs}
            output_dict = {
...
class MultiStagesDistillationBox(DistillationBox):
    ...
    def post_process(self, **kwargs):
        ...
        if (
            self.current_epoch == self.stage_end_epoch
