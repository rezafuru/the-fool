...
def wrap_if_distributed(model, device, device_ids, distributed):
    ...
    if distributed:
        ...
        return DistributedDataParallel(
            model, device_ids=device_ids, find_unused_parameters=any_frozen
        )
    return model
...
def save_module_ckpt(module, ckpt_file_path):
    ...
    state_dict = (
        module.module.state_dict() if check_if_wrapped(module) else module.state_dict()
    )
...
def redesign_model(org_model, model_config, model_label, model_type="original"):
    ...
    if not isinstance(module_paths, list) or len(module_paths) == 0:
        ...
        for frozen_module_path in frozen_module_path_set:
            if frozen_module_path.startswith(
                isinstance_str
            ) and frozen_module_path.endswith(")"):
    ...
    for module_path in module_paths:
        if module_path.startswith("+"):
            ...
            module = get_adaptation_module(
                adaptation_config["type"], **adaptation_config["params"]
            )
        else:
