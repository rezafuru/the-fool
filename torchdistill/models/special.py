...
from torchdistill.models.util import (
    wrap_if_distributed,
    load_module_ckpt,
    save_module_ckpt,
    redesign_model,
...
class Paraphraser4FactorTransfer(nn.Module):
    ...
    def __init__(
        self, k, num_input_channels, kernel_size=3, stride=1, padding=1, uses_bn=True
    ):
        super().__init__()
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.teacher_model = redesign_model(
            teacher_ref_model, minimal, "teacher", model_type
        )
        self.input_module_path = input_module_path
        self.paraphraser = wrap_if_distributed(
            Paraphraser4FactorTransfer(**paraphraser_params),
            device,
            device_ids,
            distributed,
        )
        ...
        if os.path.isfile(self.ckpt_file_path):
            map_location = (
                {"cuda:0": "cuda:{}".format(device_ids[0])} if distributed else device
            )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.student_model = wrap_if_distributed(
            student_model, device, device_ids, distributed
        )
        self.input_module_path = input_module_path
        self.translator = wrap_if_distributed(
            Translator4FactorTransfer(**translator_params),
            device,
            device_ids,
            distributed,
        )
...
class Connector4DAB(SpecialModule):
    ...
    def __init__(
        self, student_model, connectors, device, device_ids, distributed, **kwargs
    ):
        super().__init__()
        self.student_model = wrap_if_distributed(
            student_model, device, device_ids, distributed
        ...
        for connector_key, connector_params in connectors.items():
            connector = self.build_connector(
                connector_params["conv_params"], connector_params.get("bn_params", None)
            )
            self.connector_dict[connector_key] = wrap_if_distributed(
                connector, device, device_ids, distributed
            )
            io_path_pairs.append(
                (connector_key, connector_params["io"], connector_params["path"])
            )
...
class Regressor4VID(nn.Module):
    def __init__(
        self, in_channels, middle_channels, out_channels, eps, init_pred_var, **kwargs
    ):
        super().__init__()
        ...
        self.soft_plus_param = nn.Parameter(
            np.log(np.exp(init_pred_var - eps) - 1.0) * torch.ones(out_channels)
        )
        self.eps = eps
        self.init_pred_var = init_pred_var

    def forward(self, student_feature_map):
        # pred mean is a learned function (small cnn with 1x1 convs)
        pred_mean = self.regressor(student_feature_map)
        # note: pred_var is still a "constant" and not a function of the input
        #  but the parameters are learned for "alpha" in the soft plus function
        #  idk why we cannot just use unparameterized softplus though
...
class VariationalDistributor4VID(SpecialModule):
    ...
    def __init__(
        self, student_model, regressors, device, device_ids, distributed, **kwargs
    ):
        super().__init__()
        self.student_model = wrap_if_distributed(
            student_model, device, device_ids, distributed
        ...
        for regressor_key, regressor_params in regressors.items():
            ...
            self.regressor_dict[regressor_key] = wrap_if_distributed(
                regressor, device, device_ids, distributed
            )
            io_path_pairs.append(
                (regressor_key, regressor_params["io"], regressor_params["path"])
            )
        self.io_path_pairs = io_path_pairs
...
class Linear4CCKD(SpecialModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not is_teacher:
            student_model = wrap_if_distributed(
                student_model, device, device_ids, distributed
            )
        ...
        self.linear = wrap_if_distributed(
            nn.Linear(**linear_params), device, device_ids, distributed
        )
    ...
    def post_forward(self, io_dict):
        flat_outputs = torch.flatten(
            io_dict[self.input_module_path][self.input_module_io], 1
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.normalizer = wrap_if_distributed(
            Normalizer4CRD(linear, power=power), device, device_ids, distributed
        )
...
class SSWrapper4SSKD(SpecialModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.ss_module = (
            ss_module
            if is_teacher and freezes_ss_module
...
class VarianceBranch4PAD(SpecialModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.var_estimator = wrap_if_distributed(
            var_estimator, device, device_ids, distributed
        )
    ...
    def __init__(self, in_channel, mid_channel, out_channel, uses_attention):
        ...
        self.attention_conv = (
            ...
            if not uses_attention
                ...
                nn.Conv2d(mid_channel * 2, 2, kernel_size=1), nn.Sigmoid()
            )
        )
...
class Student4KnowledgeReview(SpecialModule):
    ...
    def __init__(
        self, student_model, abfs, device, device_ids, distributed, sizes=None, **kwargs
    ):
        super().__init__()
        self.student_model = wrap_if_distributed(
            student_model, device, device_ids, distributed
        ...
        for idx, abf_config in enumerate(abfs):
            abf = wrap_if_distributed(
                AttentionBasedFusion(
                    uses_attention=idx < num_abfs - 1, **abf_config["params"]
                ),
                device,
                device_ids,
                distributed,
    ...
    def post_forward(self, io_dict):
        feature_maps = [
            io_dict[module_path][io_type] for io_type, module_path in self.io_path_pairs
        ]
        out_features, res_features = self.abf_modules[0](feature_maps[0])
        if len(self.sizes) > 1:
            for features, abf, size in zip(
                feature_maps[1:], self.abf_modules[1:], self.sizes[1:]
            ):
...
class Student4KTAAD(SpecialModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.feature_adapter = wrap_if_distributed(
            feature_adapter, device, device_ids, distributed
        )
        self.affinity_adapter = wrap_if_distributed(
            affinity_adapter, device, device_ids, distributed
        )
...
def build_special_module(model_config, **kwargs):
    ...
    if special_model_type is not None:
        ...
        return get_special_module(
            special_model_type, **kwargs, **special_model_params_config
        )
    return None
