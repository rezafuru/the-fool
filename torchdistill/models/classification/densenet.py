...
MODEL_URL_DICT = {
    "cifar10-densenet_bc_k12_depth100": ROOT_URL
    + "/v0.1.1/cifar10-densenet_bc_k12_depth100.pt",
    "cifar100-densenet_bc_k12_depth100": ROOT_URL
    + "/v0.1.1/cifar100-densenet_bc_k12_depth100.pt",
}
...
def densenet(
    ...
    **kwargs: Any
    ...
    model = DenseNet4Cifar(
        growth_rate, (n, n, n), num_init_features, num_classes=num_classes, **kwargs
    )
    base_model_name = "densenet_bc" if bottleneck else "densenet"
    model_key = "cifar{}-{}_k{}_depth{}".format(
        num_classes, base_model_name, growth_rate, depth
    )
    if pretrained and model_key in MODEL_URL_DICT:
        state_dict = torch.hub.load_state_dict_from_url(
            MODEL_URL_DICT[model_key], progress=progress
        )
...
@register_model_func
def densenet_bc_k12_depth100(
    num_classes=10, pretrained=False, progress=True, **kwargs: Any
) -> DenseNet4Cifar:
