...
JpegCompressedTensor = namedtuple(
    "JpegCompressedTensor", ["tensor_buffer", "scale", "zero_point"]
)
CLASS_DICT = dict()
...
class Quantizer(object):
    ...
    def __call__(self, z):
        return (
            z.half()
            if self.num_bits == 16
            else tensor_util.quantize_tensor(z, self.num_bits)
        )
...
class JpegCompressor(object):
    ...
    def compress(self, z, output_file_path):
        ...
        return JpegCompressedTensor(
            tensor_buffer=buffer, scale=qz.scale, zero_point=qz.zero_point
        )
        ...
        for i, sub_z in enumerate(z):
            ...
            if self.tmp_dir_path is not None:
                file_path = os.path.join(
                    self.tmp_dir_path, "{}.jpg".format(hash(sub_z))
                )
...
class JpegDecompressor(object):
    ...
    def decompress(self, jc_tensor):
        if self.prints_file_size:
            logger.info(
                "{:.4f} [KB]".format(file_util.get_binary_object_size(jc_tensor))
            )
        img = Image.open(jc_tensor.tensor_buffer).convert("RGB")
        return jc_tensor.scale * (
            functional.to_tensor(img) * 255.0 - jc_tensor.zero_point
        )
...
def get_bottleneck_processor(class_name, *args, **kwargs):
    if class_name not in CLASS_DICT:
        logger.info(
            "No bottleneck processor called `{}` is registered.".format(class_name)
        )
        return None
