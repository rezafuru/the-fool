...
class Bottleneck4Inception3(BottleneckBase):
    ...
    def __init__(
        ...
        modules = [
            ...
            nn.ReLU(inplace=False),
            nn.Conv2d(
                64, bottleneck_channel, kernel_size=2, stride=2, padding=1, bias=False
            ),
            nn.BatchNorm2d(bottleneck_channel),
            nn.ReLU(inplace=False),
            nn.Conv2d(
                bottleneck_channel, 256, kernel_size=2, stride=1, padding=1, bias=False
            ),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=False),
    ...
    def __init__(self, bottleneck, short_module_names, org_model):
        ...
        for child_name, child_module in org_model.named_children():
            if child_name in short_module_set:
                if (
                    len(child_name_list) > 0
                    and child_name_list[-1] == "Conv2d_2b_3x3"
                    ...
                    len(child_name_list) > 0
                    and child_name_list[-1] == "Conv2d_4a_3x3"
...
def custom_inception_v3(
    ...
    **kwargs
    ...
    if compressor is not None:
        compressor = get_bottleneck_processor(
            compressor["name"], **compressor["params"]
        )

    if decompressor is not None:
        decompressor = get_bottleneck_processor(
            decompressor["name"], **decompressor["params"]
        )

    bottleneck = Bottleneck4Inception3(
        bottleneck_channel, bottleneck_idx, compressor, decompressor
    )
