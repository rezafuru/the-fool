...
from torchdistill.models.custom.bottleneck.detection.resnet_backbone import (
    custom_resnet_fpn_backbone,
)
    ...
    **kwargs
    ...
    if pretrained and backbone_name.endswith("resnet50"):
        state_dict = load_state_dict_from_url(
            fasterrcnn_model_urls["fasterrcnn_resnet50_fpn_coco"], progress=progress
        )
...
def custom_maskrcnn_resnet_fpn(
    ...
    **kwargs
    ...
    if pretrained and backbone_name.endswith("resnet50"):
        state_dict = load_state_dict_from_url(
            maskrcnn_model_urls["maskrcnn_resnet50_fpn_coco"], progress=progress
        )
        model.load_state_dict(state_dict, strict=False)
    return model
...
@register_model_func
def custom_keypointrcnn_resnet_fpn(
    backbone,
    pretrained=True,
    progress=True,
    num_classes=2,
    ...
    **kwargs
    ...
    if pretrained and backbone_name.endswith("resnet50"):
        state_dict = load_state_dict_from_url(
            keypointrcnn_model_urls["keypointrcnn_resnet50_fpn_coco"], progress=progress
        )
        model.load_state_dict(state_dict, strict=False)
    return model
