...
class CustomLoss(nn.Module):
    def __init__(self, criterion_config):
        ...
        if sub_terms_config is not None:
            for loss_name, loss_config in sub_terms_config.items():
                ...
                sub_criterion = get_single_loss(
                    sub_criterion_config, loss_config.get("params", None)
                )
    ...
    def __str__(self):
        ...
        desc += " + ".join(
            [
                "{} * {}".format(factor, criterion)
                for criterion, factor in self.term_dict.values()
            ]
        )
        return desc
...
class GeneralizedCustomLoss(CustomLoss):
    ...
    def forward(self, output_dict, org_loss_dict, targets):
        ...
        for loss_name, (criterion, factor) in self.term_dict.items():
            loss_dict[loss_name] = factor * criterion(
                student_output_dict, teacher_output_dict, targets
            )

        sub_total_loss = (
            sum(loss for loss in loss_dict.values()) if len(loss_dict) > 0 else 0
        )
        ...
        if isinstance(self.org_loss_factor, dict):
            org_loss = sum(
                [self.org_loss_factor[k] * v for k, v in org_loss_dict.items()]
            )
            return sub_total_loss + org_loss
        return sub_total_loss + self.org_loss_factor * sum(
            org_loss_dict.values() if len(org_loss_dict) > 0 else []
        )

    def __str__(self):
        desc = "Loss = "
        tuple_list = (
            [(self.org_loss_factor, "OrgLoss")]
            if self.org_loss_factor is not None and self.org_loss_factor != 0
            else list()
        )
        tuple_list.extend(
            [(factor, criterion) for criterion, factor in self.term_dict.values()]
        )
        desc += " + ".join(
            ["{} * {}".format(factor, criterion) for factor, criterion in tuple_list]
        )
        return desc


@register_custom_loss
class LazyGeneralizedCustomLoss(GeneralizedCustomLoss):
    # like GeneralziedCustomLoss but don't sum individual parts together
    def __init__(self, criterion_config):
        super().__init__(criterion_config)
        assert not isinstance(self.org_loss_factor, dict), "Not supported -> Check 'GeneralizedCustomLoss'"
    ...
    def forward(self, output_dict, org_loss_dict, targets):
        ...
        if self.org_loss_factor is None or (
            ...
            return loss_dict
        loss_dict.update(org_loss_dict)
        # loss_dict["org_loss"] = self.org_loss_factor * sum(
        #     org_loss_dict.values() if len(org_loss_dict) > 0 else []
        # )
        return loss_dict
...
@register_custom_loss
class LazyGeneralizedCustomLossNoKd(GeneralizedCustomLoss):
    def __init__(self, criterion_config):
        super().__init__(criterion_config)
        assert not isinstance(self.org_loss_factor, dict), "Not supported -> Check 'GeneralizedCustomLoss'"
