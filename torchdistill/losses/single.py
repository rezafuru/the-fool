...
class SimpleLossWrapper(nn.Module):
    def __init__(self, single_loss, params_config):
        ...
        self.input_module_path = input_config.get("module_path")
        self.input_key = input_config.get("io")
        target_config = params_config.get("target", dict())
        self.is_target_from_teacher = target_config.get("is_from_teacher", False)
        self.target_module_path = target_config.get("module_path")
        self.target_key = target_config.get("io")
    ...
    def forward(self, student_io_dict, teacher_io_dict, targets, *args, **kwargs):
        input_batch = self.extract_value(
            teacher_io_dict if self.is_input_from_teacher else student_io_dict,
            self.input_module_path,
            self.input_key,
        )
...
class KDLoss(nn.KLDivLoss):
    ...
    def __init__(
        self, temperature, alpha=None, beta=None, reduction="batchmean", **kwargs
    ):
...
class FSPLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        ...
        for pair_name, pair_config in self.fsp_pairs.items():
            student_first_feature_map = extract_feature_map(
                student_io_dict, pair_config["student_first"]
            )
            student_second_feature_map = extract_feature_map(
            ...
            teacher_first_feature_map = extract_feature_map(
                teacher_io_dict, pair_config["teacher_first"]
            )
            teacher_second_feature_map = extract_feature_map(
            ...
            fsp_loss += (
                factor * (student_fsp_matrices - teacher_fsp_matrices).norm(dim=1).sum()
            )
...
class ATLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        ...
        for pair_name, pair_config in self.at_pairs.items():
            student_feature_map = extract_feature_map(
                student_io_dict, pair_config["student"]
            )
            teacher_feature_map = extract_feature_map(
                teacher_io_dict, pair_config["teacher"]
            )
            factor = pair_config.get("factor", 1)
            if self.mode == "paper":
                at_loss += factor * self.compute_at_loss_paper(
                    student_feature_map, teacher_feature_map
                )
            else:
                at_loss += factor * self.compute_at_loss(
                    student_feature_map, teacher_feature_map
                )
            if batch_size is None:
...
class PKTLoss(nn.Module):
    ...
    def cosine_similarity_loss(self, student_outputs, teacher_outputs):
        ...
        student_similarity = student_similarity / torch.sum(
            student_similarity, dim=1, keepdim=True
        )
        teacher_similarity = teacher_similarity / torch.sum(
            teacher_similarity, dim=1, keepdim=True
        )
        ...
        return torch.mean(
            teacher_similarity
            * torch.log(
                (teacher_similarity + self.eps) / (student_similarity + self.eps)
            )
        )

    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        student_penultimate_outputs = student_io_dict[self.student_module_path][
            self.student_module_io
        ]
        teacher_penultimate_outputs = teacher_io_dict[self.teacher_module_path][
            self.teacher_module_io
        ]
        return self.cosine_similarity_loss(
            student_penultimate_outputs, teacher_penultimate_outputs
        )
...
class FTLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        paraphraser_flat_outputs = teacher_io_dict[self.paraphraser_path][
            "output"
        ].flatten(1)
        translator_flat_outputs = student_io_dict[self.translator_path][
            "output"
        ].flatten(1)
        norm_paraphraser_flat_outputs = (
            paraphraser_flat_outputs / paraphraser_flat_outputs.norm(dim=1).unsqueeze(1)
        )
        norm_translator_flat_outputs = (
        ...
        ft_loss = torch.norm(
            norm_translator_flat_outputs - norm_paraphraser_flat_outputs,
            self.norm_p,
            dim=1,
        )
...
class RKDLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        teacher_flat_outputs = teacher_io_dict[self.teacher_output_path][
            "output"
        ].flatten(1)
        student_flat_outputs = student_io_dict[self.student_output_path][
            "output"
        ].flatten(1)
...
class VIDLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        vid_loss = 0
        for pair_name, pair_config in self.feature_pairs.items():
            pred_mean, pred_var = extract_feature_map(
                student_io_dict, pair_config["student"]
            )
            teacher_feature_map = extract_feature_map(
                teacher_io_dict, pair_config["teacher"]
            ...
            factor = pair_config.get("factor", 1)
            neg_log_prob = 0.5 * (
                (pred_mean - teacher_feature_map) ** 2 / pred_var + torch.log(pred_var)
            )
...
class CRDLoss(nn.Module):
    ...
    def __init__(
        self,
        student_norm_module_path,
        student_empty_module_path,
        teacher_norm_module_path,
        input_size,
        output_size,
        ...
        self.register_buffer(
            "params",
            torch.tensor([num_negative_samples, temperature, -1, -1, momentum]),
        )
        stdv = 1.0 / math.sqrt(input_size / 3)
        self.register_buffer(
            "memory_v1", torch.rand(output_size, input_size).mul_(2 * stdv).add_(-stdv)
        )
        self.register_buffer(
            "memory_v2", torch.rand(output_size, input_size).mul_(2 * stdv).add_(-stdv)
        )
    ...
    def contrast_memory(
        self, student_embed, teacher_embed, pos_indices, contrast_idx=None
    ):
        ...
        if contrast_idx is None:
            contrast_idx = self.draw(batch_size * (self.num_negative_samples + 1)).view(
                batch_size, -1
            )
        ...
        weight_v1 = torch.index_select(
            self.memory_v1, 0, contrast_idx.view(-1)
        ).detach()
        ...
        weight_v2 = torch.index_select(
            self.memory_v2, 0, contrast_idx.view(-1)
        ).detach()
    ...
    def compute_contrast_loss(self, x):
        ...
        log_d0 = torch.div(
            p_neg.clone().fill_(m * pn), p_neg.add(m * pn + self.eps)
        ).log_()
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        ...
        """
        teacher_linear_outputs = teacher_io_dict[self.teacher_norm_module_path][
            "output"
        ]
        student_linear_outputs = student_io_dict[self.student_norm_module_path][
            "output"
        ]
...
class AuxSSKDLoss(nn.CrossEntropyLoss):
    ...
    def __init__(
        self, module_path="ss_module", module_io="output", reduction="mean", **kwargs
    ):
...
class SSKDLoss(nn.Module):
    ...
    def __init__(
        ...
        student_ss_module_path,
        teacher_ss_module_path,
        kl_temp,
        ss_temp,
        tf_temp,
        ss_ratio,
        ...
        student_ss_module_io="output",
        teacher_ss_module_io="output",
        loss_weights=None,
        reduction="batchmean",
        **kwargs
    ):
        super().__init__()
        self.loss_weights = (
            [1.0, 1.0, 1.0, 1.0] if loss_weights is None else loss_weights
        )
    ...
    def forward(self, student_io_dict, teacher_io_dict, targets, *args, **kwargs):
        student_linear_outputs = student_io_dict[self.student_linear_module_path][
            self.student_linear_module_io
        ]
        teacher_linear_outputs = teacher_io_dict[self.teacher_linear_module_path][
            self.teacher_linear_module_io
        ]
        device = student_linear_outputs.device
        ...
        kl_loss = self.kldiv_loss(
            torch.log_softmax(
                student_linear_outputs[normal_indices] / self.kl_temp, dim=1
            ),
        ...
        ranks = torch.argmax(
            torch.eq(ranks, aug_targets.unsqueeze(1)).long(), dim=1
        )  # groundtruth label's rank
        ...
        student_ss_module_outputs = student_io_dict[self.student_ss_module_path][
            self.student_ss_module_io
        ]
        teacher_ss_module_outputs = teacher_io_dict[self.teacher_ss_module_path][
            self.teacher_ss_module_io
        ]

        s_cos_similarities = self.compute_cosine_similarities(
            student_ss_module_outputs,
            normal_indices,
            aug_indices,
            three_forth_batch_size,
            one_forth_batch_size,
        ...
        ss_loss = self.kldiv_loss(
            torch.log_softmax(
                s_cos_similarities[distill_index_ss] / self.ss_temp, dim=1
            ),
        ...
        total_loss = 0
        for loss_weight, loss in zip(
            self.loss_weights, [ce_loss, kl_loss, ss_loss, tf_loss]
        ):
...
class PADL2Loss(nn.Module):
    ...
    def __init__(
        self,
        student_embed_module_path,
        teacher_embed_module_path,
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        ...
        student_embed_outputs = student_io_dict[self.student_embed_module_path][
            self.student_embed_module_io
        ].flatten(1)
        teacher_embed_outputs = teacher_io_dict[self.teacher_embed_module_path][
            self.teacher_embed_module_io
        ].flatten(1)
...
class HierarchicalContextLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        student_features, _ = student_io_dict[self.student_module_path][
            self.student_module_io
        ]
        teacher_features = teacher_io_dict[self.teacher_module_path][
            self.teacher_module_io
        ]
...
class RegularizationLoss(nn.Module):
    def __init__(
        self, module_path, io_type="output", is_from_teacher=False, p=1, **kwargs
    ):
        super().__init__()
        self.module_path = module_path
...
class KTALoss(nn.Module):
    ...
    def __init__(
        ...
        q=2,
        reduction="mean",
        knowledge_translator_path="paraphraser",
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        knowledge_translator_flat_outputs = teacher_io_dict[
            self.knowledge_translator_path
        ]["output"].flatten(1)
        feature_adapter_flat_outputs = student_io_dict[self.feature_adapter_path][
            "output"
        ].flatten(1)
        ...
        norm_feature_adapter_flat_outputs = (
            ...
            / feature_adapter_flat_outputs.norm(p=self.norm_q, dim=1).unsqueeze(1)
        )
        if self.norm_p == 1:
            return nn.functional.l1_loss(
                norm_feature_adapter_flat_outputs,
        ...
        kta_loss = torch.norm(
            norm_feature_adapter_flat_outputs - norm_knowledge_translator_flat_outputs,
            self.norm_p,
            dim=1,
        )
...
class AffinityLoss(nn.Module):
    ...
    def forward(self, student_io_dict, teacher_io_dict, *args, **kwargs):
        student_flat_outputs = student_io_dict[self.student_module_path][
            self.student_module_io
        ].flatten(2)
        teacher_flat_outputs = teacher_io_dict[self.teacher_module_path][
            self.teacher_module_io
        ].flatten(2)
        batch_size, ch_size, hw = student_flat_outputs.shape
        student_flat_outputs = student_flat_outputs / student_flat_outputs.norm(
            p=2, dim=2
        ).unsqueeze(-1)
        teacher_flat_outputs = teacher_flat_outputs / teacher_flat_outputs.norm(
            p=2, dim=2
        ).unsqueeze(-1)
        ...
        for i in range(ch_size):
            total_squared_losses += (
                (
                    torch.bmm(
                        student_flat_outputs[:, i].unsqueeze(2),
                        student_flat_outputs[:, i].unsqueeze(1),
                    )
                    - torch.bmm(
                        teacher_flat_outputs[:, i].unsqueeze(2),
                        teacher_flat_outputs[:, i].unsqueeze(1),
                    )
                )
                / hw
            ).norm(p=2, dim=(1, 2))
        return (
            total_squared_losses.mean()
            if self.reduction == "mean"
            else total_squared_losses.sum()
        )
...
def get_loss_wrapper(single_loss, params_config, wrapper_config):
    ...
    if wrapper_type in LOSS_WRAPPER_CLASS_DICT:
        return LOSS_WRAPPER_CLASS_DICT[wrapper_type](
            single_loss, params_config, **wrapper_config.get("params", dict())
        )
...
def get_single_loss(single_criterion_config, params_config=None):
    ...
    single_loss = (
        SINGLE_LOSS_CLASS_DICT[loss_type](**single_criterion_config["params"])
        if loss_type in SINGLE_LOSS_CLASS_DICT
        else get_loss(loss_type, single_criterion_config["params"])
    )
    if params_config is None:
        return single_loss
    return get_loss_wrapper(
        single_loss, params_config, params_config.get("wrapper", dict())
    )
...
def get_loss(loss_type, param_dict=None, **kwargs):
    ...
    if loss_type in SINGLE_LOSS_CLASS_DICT:
        return SINGLE_LOSS_CLASS_DICT[loss_type](**param_dict, **kwargs)
